...
next: kamae-2-star-actually-system-design
---

# \*, Actually — Project Seed (Kamae 1)

**Version:** Draft 4, closed
**Date:** April 11, 2026 (drafted) · July 3, 2026 (closed)
**Status:** Closed — open questions handed to Kamae 2

> **Closing pass (July 3, 2026).** This seed is closed as drafted, with the voice intact. Three decisions crystallized between draft 4 and closing, and they're recorded here so the chain reads true:
>
> 1. **Fully static.** The architecture direction in §7 wavered between a static build and a small application server. The static instinct wins, completely: every node × every depth pre-rendered as a static HTML fragment, HTMX swapping against file paths, Caddy serving files. The only dynamic piece is the AI portal endpoint — and the site works without it. HTMX, actually.
> 2. **One repo, extract later.** SSH, Actually is built in `ssh-actually`, whole. The `star-actually` system repo is extracted when the second domain arrives, from a working system — not designed in advance (§6 already committed to this; open question 8 answers itself).
> 3. **The chain is public.** This document and everything downstream of it live in `ho-process/`, tracked in the open repo — Sharibako precedent. The build record is part of the methodology demonstration.
>
> The eight open questions in §13 are handed to Kamae 2, which closes all of them. Earlier drafts (v1–v3) and the April project note live in `ho-process/pre-seeds/`.
...
**The name:** A triple reference. _HTMX, Actually_ — the design philosophy (you don't need React for this, and the name came from the build approach, not the other way around). _Love Actually_ — the pop culture hook and the earnestness underneath the technical exterior. And the conversational "actually" — the word you use when you're about to correct a misconception. The asterisk is a wildcard — a glob pattern. Together: anything you thought you understood but actually didn't, explained properly.

**The repos:** `ssh-actually` for the first content instance (the joke lands for those who catch it). `star-actually` for the system repo (GitHub won't allow `*` in repo names, but the spoken form works). The system name carries the extensibility. The content names carry the personality.
...
_Closing pass: all eight are handed to Kamae 2, which decides each one. They stay here as drafted — the record of what was open when the seed closed._
...
**Git visualization as navigation precedent:** The branch/return model maps to git's checkout/merge mental model. Two tools worth studying:

_GitKraken_ — vertical timeline as spine, branches as visible lateral departures, color-coded paths so you can trace a branch back to where it diverged, commit detail panel that shows what's at a point without leaving the graph. What works: the spatial logic is legible — you can see where you are, where things forked, where they merged back. What doesn't work for *, Actually: it's too dense. GitKraken is a power tool for thousands of commits across hundreds of branches. The visual noise is the point for developers — but a learner needs to see *their path* and *their neighborhood\*, not the entire graph. GitKraken's density level is the zoomed-out view you'd see when you escape the neighborhood — not the default reading experience.

_Learn Git Branching (learngitbranching.js.org)_ — closer to what \*, Actually needs. Small number of branches, clear node-and-line diagrams, commits as circles, branches labeled, HEAD marked, the whole thing fits on one screen. That's the right density: five to ten nodes visible, your position marked, branches clearly departing and returning.

**Sutra connection:** Sutra's problem is writing flow (marks, trails, echoes, threads, weaving). *, Actually's problem is knowledge navigation (nodes, depth, branches, return). The *data structure* question — how do you represent a graph of interconnected, layered, typed items — may overlap. But the interaction models are completely different: Sutra is about producing text, *, Actually is about navigating knowledge. If there's a shared foundation, it's in the node schema and graph computation, not in the UX. Worth investigating when both projects are further along. Not worth designing for now.

---

## The Vibe (jokes that are also the point)

These came out of the original conversation and they're not decoration — they're the project's personality crystallized.

**HTMX, Actually** is the design philosophy. SSH, Actually is the first thing built with it. The repo name `ssh-actually` carries both jokes at once and most people will only catch one.

**The Hitchhiker's Guide to the Galaxy** is literally a product that does what this project describes: a device that contains all knowledge, organized for the confused traveler, with the words DON'T PANIC on the cover. \*, Actually is a Hitchhiker's Guide for technical domains. You're lost, the system is incomprehensible, but this thing will explain it to you clearly and with a sense of humor about the whole situation.

**Small furry creatures from Alpha Centauri.** When HTMX is REAL HTMX and small furry creatures from Alpha Centauri are REAL small furry creatures from Alpha Centauri. The early web was a place where hyperlinked documents felt like magic because they _were_ — nobody had ruined them yet. That feeling is what \*, Actually is built to recover.

**Post-Butlerian Jihad.** There's something Dune-ian about this. The AI gets you to the door and then it's off. From there, it's pure human navigation through structured knowledge. No thinking machines between you and the understanding. The Mentats had to organize knowledge in their heads because computers were banned. \*, Actually organizes it in a graph so you don't have to be a Mentat — but the navigation is still yours.

**Man pages should be the best thing in computing. They kind of suck.** This is the project's origin energy. The right instinct, the wrong execution. \*, Actually is what man pages would be if they were designed for learning instead of reference.

**Adobe has the fucking shittiest help in the world.** Everyone does. Google becomes help. A product shouldn't need 5,000 blogs telling you how to use it. \*, Actually is anti-gatekeeping. It makes esoteric knowledge accessible in the way that you NEED it to be accessed.
